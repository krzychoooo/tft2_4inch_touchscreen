...
void pinHiDrive( uint32_t ulPin, unsigned hiDrive)
{
  if(hiDrive) hiDrive = 1;
  PORT->Group[g_APinDescription[ulPin].ulPort].PINCFG[g_APinDescription[ulPin].ulPin].bit.DRVSTR = hiDrive ;
}
...
TSPoint TouchScreen::getPoint(void) {
  ...
  pinMode(_yp, INPUT);
  pinMode(_ym, INPUT);
  pinMode(_xp, OUTPUT_7mA);
  pinMode(_xm, OUTPUT_7mA);
  pinMode(A0, OUTPUT);
